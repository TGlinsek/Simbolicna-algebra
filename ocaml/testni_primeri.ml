...
let izpisi_poenostavitev : unit =
    let rec aux seznam_izrazov =
        ...
        | [] -> ()
        | i :: is -> 
            let sparsan = parsaj_v_izraz i
            in
            (sparsan |> izraz_to_string) ^ " --> " ^ (sparsan |> poenostavitev |> izraz_to_string) |> print_endline;
            flush stdout;
            aux is
    in
...
let izpisi_faktorizacijo : unit =
    let rec aux seznam_izrazov =
        ...
        | (i, n) :: is -> 
            let sparsan = parsaj_v_izraz i
            in
            (sparsan |> izraz_to_string) ^ " --> " ^ (n |> (sparsan |> poenostavitev |> faktorizacija) |> fst |> izraz_to_string) |> print_endline;
            flush stdout;
            aux is
    in
