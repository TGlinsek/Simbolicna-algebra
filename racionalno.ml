...
let kanonicna_racionalno (r : racionalno) : racionalno =  (* vedno je imenovalec rezultata pozitivno število *)
...
let ( <%< ) (r : racionalno) (s : racionalno) : bool = 
    let r' = kanonicna_racionalno r in
    let s' = kanonicna_racionalno s in
    r'.stevec * s'.imenovalec < s'.stevec * r'.imenovalec  (* vedno mora biti kanonična, zaradi predznakov *)
...
let rat_v_int_opt (r : racionalno) : int option = if rat_je_int r then Some (r.stevec / r.imenovalec) else None

let rat_v_int (r : racionalno) : int =
    match rat_v_int_opt r with
    | Some x -> x
    | None -> failwith "To ni celo število"
...
let rec pow_int (m : int) (n : int) : int =
    match n with
    | 0 -> 1
    | n' when n' > 0 -> m * pow_int m (n' - 1)
    | _ -> failwith "Negativno število"

let rec pow_rat (r : racionalno) (n : int) : racionalno = 
    if n >= 0 then
        {stevec = pow_int r.stevec n; imenovalec = pow_int r.imenovalec n}
    else
        pow_rat (inverse_rat r) (-n)



let gcd_rat (r : racionalno) (s : racionalno) : racionalno =
    if r.imenovalec = 0 then failwith "TODO"
    else (
        if s.imenovalec = 0 then failwith "TODO"
        else
            {stevec = gcd (r.stevec * s.imenovalec) (s.stevec * r.imenovalec);
            imenovalec = r.imenovalec * s.imenovalec}
    )


let gcd_rat_seznam (sez : racionalno list) : racionalno =
    match sez with
    | [] -> int_v_rat 1
    | _ -> List.fold_left (fun acc r -> (gcd_rat acc r)) (int_v_rat 0) sez

let izpisi_racionalno (r : racionalno) = 
    if rat_je_int r then
        Int.to_string (r.stevec / r.imenovalec)  (* celoštevilsko deljenje *)
    else
        let r' = kanonicna_racionalno r
        in
        Int.to_string r'.stevec ^ "/" ^ Int.to_string r'.imenovalec


let float_v_rac (f : float) : racionalno =
    let s = String.split_on_char '.' (string_of_float f)
    in
    match s with
    | [a; b] -> (int_v_rat (int_of_string a)) +%+ {stevec = int_of_string b; imenovalec = pow_int 10 (String.length b)}
    | _ -> failwith "Napaka pri parsanju floata"